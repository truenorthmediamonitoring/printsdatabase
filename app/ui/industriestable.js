...
import { Pencil, Factory, X } from "lucide-react"
...
export default function IndustriesTable({ Industries }) {
    ...
    const handleFormSubmit = async (event) => {
        ...
        const endpoint = "/api/editindustry";
                        ...
                        <tbody>
                            {sortedIndustries.map((item) => (
                                <tr key={item._id} className="text-black text-xs hover:bg-zinc-100">
                                    <td className="font-bold">{item.industry}</td>
                                    <td>
                                        <div className="flex justify-end items-center gap-1">
                                            ...
                                            <button
                                                onClick={
                                                    () => {
                                                        document.getElementById('edit_industry_modal').showModal();
                                                        setindustry({
                                            ...
                                            <dialog id="edit_industry_modal" className="modal">
                                                <div className="modal-box w-80 bg-zinc-50">
                                                    ...
                                                    <div className="mb-5">
                                                        <h1 className="text-xl text-black font-notosans font-bold leading-none">Edit Industry</h1>
                                                        <span className="label-text text-xs text-zinc-500">{industry.id}</span>
                                                    </div>
                                                    {/* Form  */}
                                                    ...
                                                    <ToastAlert
                                                        stateVar={industryedited}
                                                        textColor="text-army-green"
                                                        text="Industry edited."
                                                        onClick={() => setindustryedited(false)}
                                                        iconHint="success"
                                                    />
