...
import { getOnlinePublications } from "../lib/getPublicationsOnline"
import Image from "next/image"
import DelPublicationOnlineForm from "./delpublicationonlineform";

export default async function PublicationsOnlineTable() {
    const data = await getOnlinePublications();
    // console.log(data);
                        ...
                        {Publications && Publications.reverse().map((item) => (
                            <tr key={item._id} className="text-black text-xs hover:bg-zinc-100">
                                ...
                                <td className="flex justify-end items-center">
                                    ...
                                    <DelPublicationOnlineForm id={item._id} />
                                </td>
                            </tr>
                        ))}
                    </tbody>
