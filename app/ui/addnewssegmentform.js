...
export default function AddNewsSegmentForm() {
    const [adding, setadding] = useState(false)
    const [newssegmentadded, setnewssegmentadded] = useState(false)
    ...
    const handleFormSubmit = async (event) => {
        ...
        const endpoint = "/api/savenewssegment";
        ...
        <>
            <form onSubmit={handleFormSubmit}>
                <div className="grow mb-4">
                    <div className="label">
                        <span className="label-text font-normal text-black">News Segment</span>
                    </div>
                    <label className="input input-sm flex w-full md:max-w-2xl items-center gap-2 bg-zinc-100 rounded-md shadow-sm">
                        <RadioTower size={15} className="text-red-700" />
                        <input name="newssegment" type="text" className="grow font-semibold text-black" placeholder="News segment name" required />
                    </label>
                </div>
            ...
            <ToastAlert
                stateVar={newssegmentadded}
                textColor="text-army-green"
                text="News Segment added."
                onClick={() => setnewssegmentadded(false)}
                iconHint="success"
            />
            <ToastAlert
