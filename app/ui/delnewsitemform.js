...
export default function DelNewsitemForm({ id }) {
    const [deleting, setdeleting] = useState(false);
    const [deleted, setdeleted] = useState(false);
    const [internalerror, setinternalerror] = useState(false);

    const handleNewsItemDeletion = async (event) => {
        ...
        const endpoint = "/api/deletenewsitem";
    ...
    return (
        <>
            <form onSubmit={handleNewsItemDeletion} className="flex justify-center items-center">
                {deleting === false && (
                    <button
                        name="id"
                        value={id}
